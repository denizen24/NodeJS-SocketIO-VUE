...
io.on('connection', socket => {
    ...
    socket.on('join', (user, callback) => {
        ...
        socket.emit('message:new', message('ChatBot', `Welcome, ${user.name}!`))
        socket.broadcast.to(user.room).emit('message:new', message('ChatBot', `${user.name} joined.` ))
    })

    socket.on('message:create', (data, callback) => {
        if (!data) {
            callback(`Message cant be empty`)
        } else {
    ...
    socket.on('disconnect', () => {
        ...
        if (user) {
            io.to(user.room).emit('message:new', message('ChatBot', `${user.name}, left the chat.`))
            io.to(user.room).emit('users:update', users.getByRoom(user.room))
        }
    })
})
